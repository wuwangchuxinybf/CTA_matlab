...
Date=data.NTradeDate;         %日期时间
Open=data.open;               %开盘价
High=data.high;               %最高价
Low=data.low;                %最低价
Close=data.close;              %收盘价
Volume=data.volume;             %成交量
OpenInterest=data.oi;       %持仓量
...
%策略变量
UpperLine=zeros(size(data.Ticker));               %上轨
LowerLine=zeros(size(data.Ticker));               %下轨
MidLine=zeros(size(data.Ticker));                 %中间线
Std=zeros(size(data.Ticker));                     %标准差序列
RocValue=zeros(size(data.Ticker));                %ROC值


%交易记录变量
MyEntryPrice=zeros(size(data.Ticker));            %买卖价格
MarketPosition=0;                              %仓位状态，-1表示持有空头，0表示无持仓，1表示持有多头
pos=zeros(size(data.Ticker));                     %记录仓位情况，-1表示持有空头，0表示无持仓，1表示持有多头
Type=zeros(size(data.Ticker));                    %买卖类型，1标示多头，-1标示空头
OpenPosPrice=zeros(size(data.Ticker));            %记录建仓价格
ClosePosPrice=zeros(size(data.Ticker));           %记录平仓价格
OpenPosNum=0;                                  %建仓价格序号
ClosePosNum=0;                                 %平仓价格序号
OpenDate=zeros(size(data.Ticker));            %建仓时间
CloseDate=zeros(size(data.Ticker));           %平仓时间
NetMargin=zeros(size(data.Ticker));               %净利
CumNetMargin=zeros(size(data.Ticker));            %累计净利
RateOfReturn=zeros(size(data.Ticker));            %收益率
CumRateOfReturn=zeros(size(data.Ticker));         %累计收益率
CostSeries=zeros(size(data.Ticker));              %记录交易成本
BackRatio=zeros(size(data.Ticker));               %记录回测比例

%记录资产变化变量
LongMargin=zeros(size(data.Ticker));              %多头保证金
ShortMargin=zeros(size(data.Ticker));             %空头保证金
Cash=repmat(1e6,size(data.Ticker));               %可用资金,初始资金为100W
DynamicEquity=repmat(1e6,size(data.Ticker));      %动态权益,初始资金为100W
StaticEquity=repmat(1e6,size(data.Ticker));       %静态权益,初始资金为100W
